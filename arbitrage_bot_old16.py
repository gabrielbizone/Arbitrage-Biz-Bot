...
from tokens_config import get_default_tokens_for_chain, get_base_token_for_chain, token_symbol
...
async def buscar_arbitragem_simples_base_async(session, base_token, other_tokens, amount_in_base, chain_id, log_thr, alert_thr, fee_bps, collector, notifier, sanity_cap, recheck_thr, recheck_tol):
    ...
    for token_b in other_tokens:
        ...
        if recheck_thr > 0 and net >= recheck_thr:
            ...
            rq_ba = await get_best_quote_async(session, token_b, base_token, rq_ab["toAmount"], chain_id, aggregator_list=non_ps) if rq_ab else None
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(headers={
        ...
        async def run_once():
            ...
            # SUMÁRIO: agora com nome da chain e amt em USDC
            if ALWAYS_SUMMARY and found:
                top = sorted(found, key=lambda x: x[0], reverse=True)[:SUMMARY_TOP_K]
                linhas = []
                for i,(net,msg,t,cid,amt) in enumerate(top):
                    amt_usdc = amt // 10**6
                    linhas.append(f"{i+1}. {t} chain {_chain_name(cid)} amt {amt_usdc} USDC→ net {net:.2f}% | {msg.split(' ',1)[1]}")
            ...
            # HEARTBEAT: inclui os nomes das chains e amts com unidade
            if HEARTBEAT_EVERY > 0 and cycle % HEARTBEAT_EVERY == 0:
                names = [_chain_name(c) for c in CHAIN_IDS]
                amts = [a//10**6 for a in AMOUNTS_BASE]
                send_telegram(f"{HEARTBEAT_TAG}: vivo às {time.strftime('%H:%M:%S')} | chains={names} | amounts={amts} USDC | log={LOG_THR}% | alert={ALERT_THR}%")
