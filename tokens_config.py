...
# tokens_config.py — com DECIMALS e helpers
import os
...
# base token por chain (padrão USDC nativo/bridge principal)
BASE_TOKEN_BY_CHAIN = {
...
def get_token_decimals(chain_id: int, addr: str) -> int:
    return DECIMALS_BY_CHAIN.get(chain_id, {}).get(addr.lower(), 18)
