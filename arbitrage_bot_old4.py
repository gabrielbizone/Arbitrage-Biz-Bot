...
TOKENS: Dict[str, Dict[str, object]] = {
    ...
    # Added USDT (Tether) on Polygon.  USDT is a highly liquid stablecoin
    # with 6 decimals on Polygon.  Adding it to the token list broadens
    # the search for arbitrage opportunities, especially among stablecoin
    # routes like USDC → USDT → USDC.
    "USDT": {
        # Address for (PoS) Tether USD on Polygon
...
def perform_arbitrage_check() -> None:
    """Check for arbitrage opportunities across one‑ and two‑hop routes.

    This function now evaluates both simple two‑swap cycles (USDC → token → USDC)
    and more complex three‑swap cycles (USDC → tokenA → tokenB → USDC).  It uses
    a fixed principal of 1 USDC (in smallest units) for quoting.  Whenever
    a route yields a return greater than or equal to the configured
    percentage threshold, the route and its profit are collected.  All
    results are sent in a single Telegram message with a timestamp in
    ...
    profitable_results: list[tuple[str, float]] = []

    # --- One‑hop arbitrage: USDC -> token -> USDC ---
    for symbol, info in TOKENS.items():
        ...
        if profit_percent >= threshold:
            # Record route as single token
            profitable_results.append((f"{symbol}", profit_percent))

    # --- Two‑hop arbitrage: USDC -> tokenA -> tokenB -> USDC ---
    symbols = list(TOKENS.keys())
    for i, sym_a in enumerate(symbols):
        info_a = TOKENS[sym_a]
        for j, sym_b in enumerate(symbols):
            # skip identical tokens to avoid redundant single‑hop repetition
            if i == j:
                continue
            info_b = TOKENS[sym_b]

            # Step 1: USDC -> tokenA
            quote1 = quote_swap(CHAIN_ID, USDC_ADDRESS, info_a["address"], principal)
            if quote1 is None:
                continue
            try:
                amt1 = int(quote1.get("toTokenAmount", 0))
            except Exception:
                continue
            if amt1 <= 0:
                continue
            ...
            quote2 = quote_swap(CHAIN_ID, info_a["address"], info_b["address"], amt1)
            if quote2 is None:
                continue
            try:
                amt2 = int(quote2.get("toTokenAmount", 0))
            except Exception:
                continue
            if amt2 <= 0:
                continue
            ...
            # Step 3: tokenB -> USDC
            quote3 = quote_swap(CHAIN_ID, info_b["address"], USDC_ADDRESS, amt2)
            if quote3 is None:
                continue
            try:
                final_amt = int(quote3.get("toTokenAmount", 0))
            except Exception:
                continue
            if final_amt <= 0:
                continue

            profit_percent_2 = (final_amt - principal) / principal * 100.0
            if profit_percent_2 >= threshold:
                # Record as route "A->B"
                profitable_results.append((f"{sym_a}->{sym_b}", profit_percent_2))
    ...
    if profitable_results:
        # Sort by profit descending for readability
        profitable_results.sort(key=lambda x: x[1], reverse=True)
        lines = [f"{route}: {profit:.2f}%" for route, profit in profitable_results]
        message = (
