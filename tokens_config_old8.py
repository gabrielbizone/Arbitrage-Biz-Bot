...
# tokens_config.py — acrescenta token_symbol() (só o nome) além do token_label()
import os
...
# mapeamento inverso: addr -> símbolo
_INVERSE_BY_CHAIN = {cid: {addr.lower(): sym for sym, addr in mapping.items()} for cid, mapping in TOKENS_BY_CHAIN.items()}

def token_label(addr: str, chain_id: int) -> str:
    """Mantido por compatibilidade: retorna SYMBOL[suffix] se conhecido, senão 0x…abcd."""
...
def token_symbol(addr: str, chain_id: int) -> str:
    """Novo: retorna apenas o símbolo. Se não for conhecido, retorna 'UNKNOWN'."""
    return _INVERSE_BY_CHAIN.get(chain_id, {}).get(addr.lower(), "UNKNOWN")
