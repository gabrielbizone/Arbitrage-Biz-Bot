...
import os
import random
from tokens_config import get_token_decimals
...
async def _quote_paraswap(session, chain_id, from_token, to_token, amount):
    ...
    params = {
        ...
        "amount": str(amount),
        "side": "SELL",
        # Passar decimais explícitos evita destAmount incorreto em algumas redes (ex.: BSC)
        "srcDecimals": str(get_token_decimals(chain_id, from_token)),
        "destDecimals": str(get_token_decimals(chain_id, to_token)),
    }
...
async def get_best_quote_async(session, from_token: str, to_token: str, amount: int, chain_id: int = 137, aggregator_list=None):
    # permitir override da lista para recheck/consenso
    order = aggregator_list or os.getenv("AGGREGATORS", "1inch,0x,KyberSwap,OpenOcean,Odos,ParaSwap").split(",")
    order = [x.strip() for x in order if x.strip()]
    if aggregator_list is None and os.getenv("AGGREGATORS") is None:
