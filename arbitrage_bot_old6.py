...
def buscar_arbitragem_simples(tokens, amount, chain_id):
    for token_a in tokens:
        for token_b in tokens:
            ...
            if percentual > 0.5:
                print(f"[{time.strftime('%H:%M:%S')}] SIMPLES {token_a[-4:]} → {token_b[-4:]} → {token_a[-4:]} = {percentual:.2f}% via {quote_ab['aggregator']} + {quote_ba['aggregator']}")

if __name__ == "__main__":
    print("🔁 Buscando oportunidades de arbitragem...")
