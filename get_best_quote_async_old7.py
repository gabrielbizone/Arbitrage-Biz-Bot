...
async def _quote_1inch(session, chain_id, from_token, to_token, amount):
    name = "1inch"
    # try v5 with new param names
    url1 = f"https://api.1inch.io/v5.0/{chain_id}/quote"
    params1 = {"src": from_token, "dst": to_token, "amount": str(amount)}
    data = await _fetch_json(session, "GET", url1, name, params=params1)
    if data and data.get("dstAmount") and str(data.get("dstAmount")).isdigit():
        val = int(data["dstAmount"])
        _log(f"[{name}] v5 src/dst toAmount={val}")
        return val
    # fallback v5 classic param names
    params2 = {"fromTokenAddress": from_token, "toTokenAddress": to_token, "amount": str(amount)}
    data = await _fetch_json(session, "GET", url1, name, params=params2)
    if data and data.get("toTokenAmount") and str(data.get("toTokenAmount")).isdigit():
        val = int(data["toTokenAmount"])
        _log(f"[{name}] v5 from/to toAmount={val}")
        return val
    # fallback v4 just in case
    url4 = f"https://api.1inch.io/v4.0/{chain_id}/quote"
    data = await _fetch_json(session, "GET", url4, name, params=params2)
    if data and data.get("toTokenAmount") and str(data.get("toTokenAmount")).isdigit():
        val = int(data["toTokenAmount"])
        _log(f"[{name}] v4 toAmount={val}")
        return val
    return None
...
async def _quote_kyber(session, chain_id, from_token, to_token, amount):
    ...
    # try POST route/encode (some deployments require POST)
    data = await _fetch_json(session, "POST", url, name, json=params)
    if data and data.get("amountOut") and str(data.get("amountOut")).isdigit():
        return int(data["amountOut"])
    # try routes v1 (alt path)
