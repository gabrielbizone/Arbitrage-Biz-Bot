...
# Alguns agregadores não cobrem todas as redes/tokens.
# Permitir override por ENV AGGREGATORS_<chainId>
def _aggregators_for_chain(chain_id: int):
    env_key = f"AGGREGATORS_{chain_id}"
    v = os.getenv(env_key)
    if v:
        return [x.strip() for x in v.split(",") if x.strip()]
    # default: todos; se algum não suportar, apenas não retorna quote
    return [x.strip() for x in os.getenv("AGGREGATORS", "1inch,0x,KyberSwap,OpenOcean,Odos,ParaSwap").split(",") if x.strip()]
...
async def _fetch_json(session, method, url, name, **kwargs):
    try:
        ...
        async with session.request(method, url, timeout=timeout, **kwargs) as resp:
            if resp.status != 200:
                txt = await resp.text()
                _log(f"[{name}] HTTP {resp.status} -> {url} | {txt[:200]}")
                return None
...
async def _quote_paraswap(session, chain_id, from_token, to_token, amount):
    ...
    params = {
        ...
        "amount": str(amount),
        "side": "SELL",
        # decimais podem ser passados via ENV quando necessário (opcional)
...
async def get_best_quote_async(session, from_token: str, to_token: str, amount: int, chain_id: int = 137, aggregator_list=None):
    ...
    if aggregator_list is None and os.getenv("AGGREGATORS") is None and os.getenv(f"AGGREGATORS_{chain_id}") is None:
        order = order.copy()
        random.shuffle(order)  # leve aleatoriedade para evitar rate-limit
